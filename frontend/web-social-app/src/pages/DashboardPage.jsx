...
import CreatePostModal from "../components/modals/CreatePostModal"; // Added import
...
const DashboardPage = () => {
  ...
  const [isCreatePostModalOpen, setIsCreatePostModalOpen] = useState(false); // Added state for modal
    ...
    <div className="flex flex-col h-screen bg-gray-900">
      <div className="flex flex-1 overflow-hidden">
        <Sidebar
          ...
          openCreatePostModal={() => setIsCreatePostModalOpen(true)} // New prop
        />
        <div className="flex-1 flex flex-col overflow-y-auto"> {/* Parent of main and aside (aside is fixed) */}
          <main className="flex-1 p-4 lg:mr-80"> {/* MODIFIED: removed max-w-2xl, mx-auto, space-y-6. Kept flex-1, p-4, lg:mr-80 */}
            <div className="max-w-2xl mx-auto space-y-6"> {/* NEW: Centering wrapper, added space-y-6 */}
              <div className="sticky top-0 bg-gray-900 pt-4 pb-2 z-10"> {/* MOVED: Now inside centering wrapper */}
                {user && (
              ...
              <PostList /> {/* MOVED: Now inside centering wrapper */}
            </div>
          </main>
          <aside className="hidden lg:block w-80 flex-none p-2 bg-gray-800 border-l border-gray-700 fixed top-12 right-0 h-[calc(100vh-3rem)] overflow-y-auto z-20">
            <h3 className="text-white font-bold mt-4">Personas Sugeridas</h3>
